...
it('fetches the order', async () => {
  ...
  const { body: fetchOrder } = await request(app).get(`/api/v0.0.1/orders/${order.id}`)
...
it('returns an error if one user tries to fetch other user oder', async () => {
  ...
  await request(app).get(`/api/v0.0.1/orders/${order.id}`)
